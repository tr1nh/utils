...
sudo apt install tigervnc-common tigervnc-standalone-server xorg xinit xclip xsel xdotool wmctrl \
  ...
  net-tools nmap wget curl git transmission-cli w3m openssh-server httrack \
  tree p7zip-full p7zip-rar rar unrar \
  emacs vim-gtk3 figlet toilet ffmpeg gifsicle imagemagick poppler-utils mupdf-tools gimp inkscape scrot maim jq tidy \
  mediainfo feh cmus mpv catimg pavucontrol \
  build-essential python3-pip ghostscript \
  nudoku nethack-console -y
